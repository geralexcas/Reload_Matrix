<template>
  <div class="libro-compras-container">
    <div class="page-header">
      ...
      <div class="page-actions">
        ...
        <button @click="loadLibroCompras" class="btn btn-primary" :disabled="loading">
    ...
    <div class="filters-bar">
      <div class="filter-group">
        <label>Desde</label>
        <input type="date" v-model="dateFrom" @change="loadLibroCompras" />
      </div>
      <div class="filter-group">
        <label>Hasta</label>
        <input type="date" v-model="dateTo" @change="loadLibroCompras" />
      </div>
    </div>

    <div v-if="loading" class="loading-state">
      <div class="spinner"></div>
      <p>Generando Libro de Compras...</p>
    </div>
    ...
    <div v-if="!loading && libroCompras" class="libro-content">
      <div class="company-header">
        <h3>{{ libroCompras.company_name || 'Empresa' }}</h3>
        <p>NIT: {{ libroCompras.company_nit || 'N/A' }}</p>
        <p>Período: {{ formatDateRange(libroCompras.date_from, libroCompras.date_to) }}</p>
      </div>
        ...
        <table class="compras-table">
          <thead>
            <tr>
              ...
              <th class="text-right">Total IVA Soportado</th>
          ...
          <tbody>
            <tr v-for="entry in libroCompras.entries" :key="entry.invoice_id">
          ...
          <tfoot>
            <tr class="totals-row">
              ...
              <td class="text-right"><strong>{{ formatCurrency(libroCompras.totals.total_base_iva_5) }}</strong></td>
              <td class="text-right"><strong>{{ formatCurrency(libroCompras.totals.total_iva_5) }}</strong></td>
              <td class="text-right"><strong>{{ formatCurrency(libroCompras.totals.total_base_no_iva) }}</strong></td>
              <td class="text-right"><strong>{{ formatCurrency(libroCompras.totals.total_iva) }}</strong></td>
              <td class="text-right"><strong>{{ formatCurrency(libroCompras.totals.total_retencion_fuente) }}</strong></td>
              <td class="text-right"><strong>{{ formatCurrency(libroCompras.totals.total_retencion_iva) }}</strong></td>
              <td class="text-right"><strong>{{ formatCurrency(libroCompras.totals.total_facturas) }}</strong></td>
              <td></td>
            </tr>
          </tfoot>
        </table>
      ...
      <div v-if="libroCompras.entries.length === 0" class="empty-state">
        <i class="fas fa-file-invoice"></i>
        <p>No hay facturas de compra en el período seleccionado</p>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    exportCSV() {
      if (!this.libroCompras || !this.libroCompras.entries.length) return

      let csv = 'Fecha,N° Factura,NIT Proveedor,Nombre/Base IVA 19%,IVA 19%,Base IVA 5%,IVA 5%,Sin IVA,Total IVA,Retefuente,ReteIVA,Total Factura,Estado DIAN\n'
      for (const entry of this.libroCompras.entries) {
      ...
      const t = this.libroCompras.totals
      ...
      link.download = `libro_compras_${new Date().toISOString().slice(0, 10)}.csv`
      link.click()
    }
  },
  created() {
    this.loadLibroCompras()
  }
}
</script>
...
.compras-table tfoot .totals-row {
